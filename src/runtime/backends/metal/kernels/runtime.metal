...
kernel void llm_cast_f32_f16(device const float *input [[buffer(0)]],
                             device half *output [[buffer(1)]],
                             constant ElementwiseParameters &parameters [[buffer(2)]],
                             uint vector_index [[thread_position_in_grid]]) {
    const uint base = vector_index * 4;
    if (base + 3 < parameters.count) {
        reinterpret_cast<device half4 *>(output)[vector_index] =
            half4(reinterpret_cast<device const float4 *>(input)[vector_index]);
    } else {
        for (uint index = base; index < parameters.count; ++index) {
...
kernel void llm_cast_f16_f32(device const half *input [[buffer(0)]],
                             device float *output [[buffer(1)]],
                             constant ElementwiseParameters &parameters [[buffer(2)]],
                             uint vector_index [[thread_position_in_grid]]) {
    const uint base = vector_index * 4;
    if (base + 3 < parameters.count) {
        reinterpret_cast<device float4 *>(output)[vector_index] =
            float4(reinterpret_cast<device const half4 *>(input)[vector_index]);
    } else {
        for (uint index = base; index < parameters.count; ++index) {
...
inline ushort llm_float_to_bf16(float value) {
    uint bits = as_type<uint>(value);
    const uint absolute = bits & 0x7fffffffu;
    if (absolute >= 0x7f800000u) {
        return ushort((bits >> 16u) | (absolute > 0x7f800000u ? 0x0040u : 0u));
    }
    bits += 0x7fffu + ((bits >> 16u) & 1u);
    return ushort(bits >> 16u);
}

inline float llm_bf16_to_float(ushort value) { return as_type<float>(uint(value) << 16u); }
...
kernel void llm_cast_f32_bf16(device const float *input [[buffer(0)]],
                              device ushort *output [[buffer(1)]],
                              constant ElementwiseParameters &parameters [[buffer(2)]],
                              uint vector_index [[thread_position_in_grid]]) {
    const uint base = vector_index * 4;
    if (base + 3 < parameters.count) {
        const float4 values = reinterpret_cast<device const float4 *>(input)[vector_index];
        reinterpret_cast<device ushort4 *>(output)[vector_index] =
            ushort4(llm_float_to_bf16(values.x), llm_float_to_bf16(values.y),
                    llm_float_to_bf16(values.z), llm_float_to_bf16(values.w));
    } else {
        for (uint index = base; index < parameters.count; ++index) {
            output[index] = llm_float_to_bf16(input[index]);
        }
    }
}

kernel void llm_cast_bf16_f32(device const ushort *input [[buffer(0)]],
                              device float *output [[buffer(1)]],
                              constant ElementwiseParameters &parameters [[buffer(2)]],
                              uint vector_index [[thread_position_in_grid]]) {
    const uint base = vector_index * 4;
    if (base + 3 < parameters.count) {
        const ushort4 values = reinterpret_cast<device const ushort4 *>(input)[vector_index];
        reinterpret_cast<device float4 *>(output)[vector_index] =
            float4(llm_bf16_to_float(values.x), llm_bf16_to_float(values.y),
                   llm_bf16_to_float(values.z), llm_bf16_to_float(values.w));
    } else {
        for (uint index = base; index < parameters.count; ++index) {
            output[index] = llm_bf16_to_float(input[index]);
        }
    }
}
...
kernel void llm_matmul_f16_f32(device const half *left [[buffer(0)]],
                               device const half *right [[buffer(1)]],
                               device float *output [[buffer(2)]],
                               constant MatmulParameters &parameters [[buffer(3)]],
                               uint2 group [[threadgroup_position_in_grid]],
                               uint2 local [[thread_position_in_threadgroup]]) {
    threadgroup half left_tile[16][16];
    threadgroup half right_tile[16][16];
    const uint row = group.y * llm_matmul_tile + local.y;
    const uint column = group.x * llm_matmul_tile + local.x;
    float sum = 0.0f;
    ...
    for (uint tile = 0; tile < parameters.inner_size; tile += llm_matmul_tile) {
        ...
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint inner = 0; inner < llm_matmul_tile; ++inner) {
            sum += float(left_tile[local.y][inner]) * float(right_tile[inner][local.x]);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    if (row < parameters.rows && column < parameters.columns) {
...
kernel void llm_matmul_f16_f32_tiled32(device const half *left [[buffer(0)]],
                                       device const half *right [[buffer(1)]],
                                       device float *output [[buffer(2)]],
                                       constant MatmulParameters &parameters [[buffer(3)]],
                                       uint2 group [[threadgroup_position_in_grid]],
                                       uint2 local [[thread_position_in_threadgroup]]) {
    threadgroup half left_tile[32][32];
    threadgroup half right_tile[32][32];
    const uint row0 = group.y * 32 + local.y;
    const uint row1 = row0 + 16;
    const uint column0 = group.x * 32 + local.x;
    const uint column1 = column0 + 16;
...
kernel void llm_matmul_bf16_f32(device const ushort *left [[buffer(0)]],
    ...
    for (uint tile = 0; tile < parameters.inner_size; tile += llm_matmul_tile) {
        const uint left_column = tile + local.x;
        const uint right_row = tile + local.y;
        left_tile[local.y][local.x] =
            row < parameters.rows && left_column < parameters.inner_size
                ? llm_bf16_to_float(left[row * parameters.inner_size + left_column])
                : 0.0f;
        right_tile[local.y][local.x] =
            right_row < parameters.inner_size && column < parameters.columns
                ? llm_bf16_to_float(right[right_row * parameters.columns + column])
                : 0.0f;
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint inner = 0; inner < llm_matmul_tile; ++inner) {
            sum += left_tile[local.y][inner] * right_tile[inner][local.x];
...
kernel void llm_matmul_bf16_f32_tiled32(device const ushort *left [[buffer(0)]],
                                        device const ushort *right [[buffer(1)]],
                                        device float *output [[buffer(2)]],
                                        constant MatmulParameters &parameters [[buffer(3)]],
                                        uint2 group [[threadgroup_position_in_grid]],
    ...
    for (uint tile = 0; tile < parameters.inner_size; tile += 32) {
        ...
        const uint right_row1 = right_row0 + 16;
        left_tile[local.y][local.x] =
            row0 < parameters.rows && inner0 < parameters.inner_size
                ? llm_bf16_to_float(left[row0 * parameters.inner_size + inner0])
                : 0.0f;
        left_tile[local.y][local.x + 16] =
            row0 < parameters.rows && inner1 < parameters.inner_size
                ? llm_bf16_to_float(left[row0 * parameters.inner_size + inner1])
                : 0.0f;
        left_tile[local.y + 16][local.x] =
            row1 < parameters.rows && inner0 < parameters.inner_size
                ? llm_bf16_to_float(left[row1 * parameters.inner_size + inner0])
                : 0.0f;
        left_tile[local.y + 16][local.x + 16] =
            row1 < parameters.rows && inner1 < parameters.inner_size
                ? llm_bf16_to_float(left[row1 * parameters.inner_size + inner1])
                : 0.0f;
        right_tile[local.y][local.x] =
            right_row0 < parameters.inner_size && column0 < parameters.columns
                ? llm_bf16_to_float(right[right_row0 * parameters.columns + column0])
                : 0.0f;
        right_tile[local.y][local.x + 16] =
            right_row0 < parameters.inner_size && column1 < parameters.columns
                ? llm_bf16_to_float(right[right_row0 * parameters.columns + column1])
                : 0.0f;
        right_tile[local.y + 16][local.x] =
            right_row1 < parameters.inner_size && column0 < parameters.columns
                ? llm_bf16_to_float(right[right_row1 * parameters.columns + column0])
                : 0.0f;
        right_tile[local.y + 16][local.x + 16] =
            right_row1 < parameters.inner_size && column1 < parameters.columns
                ? llm_bf16_to_float(right[right_row1 * parameters.columns + column1])
                : 0.0f;
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint inner = 0; inner < 32; ++inner) {
            const float2 left_values =
