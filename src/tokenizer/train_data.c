...
struct tokenizer_training_data {
    word_table words;
    pair_table pairs;
    pair_heap heap;
    uint64_t selection_generation;
...
static tokenizer_status word_table_rehash(word_table *table, size_t new_capacity) {
    if (tokenizer_allocation_would_overflow(new_capacity, sizeof(*table->entries))) {
...
static tokenizer_status word_table_reserve(word_table *table) {
    ...
    if (table->size + 1U < table->capacity - table->capacity / 3U) {
        return TOKENIZER_OK;
    }
...
static tokenizer_status word_table_add(word_table *table, const unsigned char *bytes,
    ...
    if (tokenizer_allocation_would_overflow(length, sizeof(*entry->ids))) {
        return TOKENIZER_OVERFLOW;
    }
...
tokenizer_status tokenizer_training_collect_pretoken(const unsigned char *bytes, size_t length,
                                                     void *context) {
    tokenizer_training_data *data = context;
    return word_table_add(&data->words, bytes, length);
}
...
static tokenizer_status pair_heap_reserve(pair_heap *heap) {
    ...
    if (new_capacity < heap->capacity ||
        tokenizer_allocation_would_overflow(new_capacity, sizeof(*heap->entries))) {
        return TOKENIZER_OVERFLOW;
    }
...
static tokenizer_status pair_entry_add_word(pair_entry *entry, size_t word_index) {
    ...
    if (entry->word_count == entry->word_capacity) {
        ...
        if (new_capacity < entry->word_capacity ||
            tokenizer_allocation_would_overflow(new_capacity, sizeof(*entry->word_indices))) {
            return TOKENIZER_OVERFLOW;
        }
...
static tokenizer_status collect_pairs(const word_table *words, pair_table *pairs,
    ...
    for (size_t word_index = 0U; word_index < words->capacity; ++word_index) {
        if (progress_callback != NULL && word_index % 4096U == 0U) {
            progress_callback(TOKENIZER_TRAIN_COLLECTING_PAIRS, word_index, words->capacity,
                              progress_context);
        }
    ...
    if (progress_callback != NULL) {
        progress_callback(TOKENIZER_TRAIN_COLLECTING_PAIRS, words->capacity, words->capacity,
                          progress_context);
    }
...
static void word_table_release_source_bytes(word_table *words) {
    for (size_t index = 0U; index < words->capacity; ++index) {
        free(words->entries[index].bytes);
        words->entries[index].bytes = NULL;
        words->entries[index].byte_length = 0U;
        words->entries[index].hash = 0U;
    }
}
...
tokenizer_status tokenizer_training_data_create(tokenizer_training_data **out_data) {
    ...
    *out_data = calloc(1U, sizeof(**out_data));
    return *out_data == NULL ? TOKENIZER_ALLOCATION_FAILED : TOKENIZER_OK;
}
...
void tokenizer_training_data_destroy(tokenizer_training_data *data) {
    ...
    pair_heap_destroy(&data->heap);
    pair_table_destroy(&data->pairs);
    word_table_destroy(&data->words);
    free(data);
}
...
tokenizer_status tokenizer_training_prepare(tokenizer_training_data *data,
    ...
    tokenizer_status status =
        collect_pairs(&data->words, &data->pairs, progress_callback, progress_context);
    word_table_release_source_bytes(&data->words);
    if (status != TOKENIZER_OK) {
        return status;
    }

    if (progress_callback != NULL) {
        progress_callback(TOKENIZER_TRAIN_BUILDING_HEAP, 0U, 1U, progress_context);
    }
    status = pair_heap_rebuild(&data->heap, &data->pairs);
    if (status == TOKENIZER_OK && progress_callback != NULL) {
        progress_callback(TOKENIZER_TRAIN_BUILDING_HEAP, 1U, 1U, progress_context);
    }
    return status;
}
...
tokenizer_status tokenizer_training_merge_next(tokenizer_training_data *data, tokenizer *tokenizer,
                                               int *out_did_merge) {
    if (data == NULL || tokenizer == NULL || out_did_merge == NULL) {
        return TOKENIZER_INVALID_ARGUMENT;
    }
    *out_did_merge = 0;

    if (data->pairs.size != 0U && data->pairs.size <= SIZE_MAX / 4U &&
        data->heap.size > data->pairs.size * 4U) {
        const tokenizer_status rebuild_status = pair_heap_rebuild(&data->heap, &data->pairs);
        if (rebuild_status != TOKENIZER_OK) {
            return rebuild_status;
        }
    }

    uint64_t best_pair = 0U;
    if (pair_heap_find_best(&data->heap, &data->pairs, &best_pair) == 0) {
        return TOKENIZER_OK;
    }

    if (data->selection_generation == UINT64_MAX) {
        return TOKENIZER_OVERFLOW;
    }
    ++data->selection_generation;
    ...
    tokenizer_status status = tokenizer_append_merge(tokenizer, left_id, right_id);
    if (status == TOKENIZER_OK) {
        status =
            apply_merge_to_affected_words(&data->words, &data->pairs, &data->heap, best_pair,
                                          left_id, right_id, merged_id, data->selection_generation);
    }
    if (status == TOKENIZER_OK) {
        *out_did_merge = 1;
    }
